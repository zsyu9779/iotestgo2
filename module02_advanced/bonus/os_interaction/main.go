...
func demoExec() {
	...
	// 4. CommandContext 到期后终止子进程。
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Millisecond)
	defer cancel()
	sleepCmd := exec.CommandContext(ctx, os.Args[0])
	sleepCmd.Env = append(os.Environ(), "MODULE02_OS_HELPER=sleep")
	err = sleepCmd.Run()
	fmt.Printf("CommandContext timeout: %v\n", ctx.Err())

	// 5. ExitError 暴露非零退出码。
	exitCmd := exec.Command(os.Args[0])
	exitCmd.Env = append(os.Environ(), "MODULE02_OS_HELPER=exit")
...
func main() {
	switch os.Getenv("MODULE02_OS_HELPER") {
	case "sleep":
