...
func demoDir() string {
	...
	if !ok {
		panic("cannot locate file I/O demo directory")
	}
...
func seekAndRead(filename string) {
	...
	buf := make([]byte, 5)
	if _, err := io.ReadAtLeast(f, buf, len(buf)); err != nil {
		panic(err)
	}
	...
	fmt.Printf("ReadAt(0): %s\n", string(buf)) // ReadAt 不改变当前偏移量。
}
