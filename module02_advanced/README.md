...
- 使用隐式接口、方法集、类型断言、错误包装和 `io.Reader` / `io.Writer` 表达行为契约；
...
| 09:50–10:40 | Block 1：接口、错误与恢复边界 | 完成接口、错误链与基础文件读写练习 |
| 10:40–10:50 | 短休息 | — |
| 10:50–12:00 | Block 2：Goroutine 与 Channel | 精讲动态并发素数过滤流水线 |
| 12:00–13:00 | 午休 | — |
...
净课堂时间为 310 分钟；Block 2 改为精讲，不设置 Starter 或课后作业。
...
| Block 2 | [Goroutine 与 Channel](blocks/02_goroutines_channels/README.md) | 无：课堂精讲 | `go run ./module02_advanced/blocks/02_goroutines_channels/demo/04_channels` |
...
OS、Runtime、标准库工具、Embed、Generate、pprof 和 DeepEqual 不占用核心 310 分钟。
